...
function MenuItem({ item }) {
    ...
    return (
        <div className=" relative flex flex-col w-auto  bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mb-8">
            ...
            <div className="px-5 pb-5">
                <div className='flex  items-center justify-between  mt-3'>
                    <Link>
                        <h5 className="text-xs md:text-lg sm:font-semibold  tracking-tight text-gray-900 dark:text-white">{name}</h5>
                    </Link>
                    {!soldOut ? <p className="text-sm sm:font-bold">{formatCurrency(unitPrice)}</p>
                        :
                        (
                            <p className="text-sm uppercase font-medium text-stone-500">Sold out</p>
                        )}
                </div>
                <span className='hidden md:flex capitalize text-sm font-light   mb-4'>{ingredients.join(', ')}</span>
